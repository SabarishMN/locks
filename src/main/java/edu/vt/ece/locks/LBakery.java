...
public class LBakery implements Lock, Timestamp, TimestampSystem {
    private final boolean[] flag;
    private final DataTimestamp[] label;
    private final long n;
    private final int L;
    private final AtomicInteger inCS;
    ...
    public void lock() {
        ...
        while (!canEnterCriticalSection((int) i, newLabel)) {
            // busy wait
//            System.out.println("Lock : " + Thread.currentThread().getId());
        }

        inCS.incrementAndGet();
    }
    ...
    private boolean canEnterCriticalSection(int i, DataTimestamp myTimestamp) {
        if (inCS.get() >= L) {
            return false;
        }

        if(inCS.get() == 0) return true;
